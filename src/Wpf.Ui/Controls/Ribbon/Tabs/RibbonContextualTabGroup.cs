...
public class RibbonContextualTabGroup : Control
{
    ...
    /// <summary>
    /// Gets the visibility this group for internal use (this enables us to hide this group when all items in this group are hidden)
    /// </summary>
    ...
    /// <summary>
    /// Initializes static members of the <see cref="RibbonContextualTabGroup"/> class.
    /// Static constructor
    /// </summary>
    ...
    public void UpdateInnerVisiblityAndGroupBorders()
    {
        ...
        for (var i = 0; i < this.Items.Count; i++)
        {
            ...
            if (this.Items[i].Visibility == Visibility.Visible
                && leftset == false)
            {
                this.Items[i].SetCurrentValue(RibbonTabItem.HasLeftGroupBorderProperty, true);
                leftset = true;
            }
            else
            {
                this.Items[i].SetCurrentValue(RibbonTabItem.HasLeftGroupBorderProperty, false);
            }

            if (this.Items[this.Items.Count - 1 - i].Visibility == Visibility.Visible
                && rightset == false)
            {
                this.Items[this.Items.Count - 1 - i].SetCurrentValue(RibbonTabItem.HasRightGroupBorderProperty, true);
                rightset = true;
            }
            else
            {
                this.Items[this.Items.Count - 1 - i].SetCurrentValue(RibbonTabItem.HasRightGroupBorderProperty, false);
            }
        }
    }
    ...
    private static void ForceRedraw( RibbonContextualTabGroup contextGroup )
    {
        ...
        foreach (RibbonTabItem ribbonTabItem in contextGroup.Items)
        {
