...
public class RibbonTabItem : Control, IHeaderedControl, ILogicalChildSupport, ISimplifiedStateControl
{
    ...
    // Collection of simplified items (auto-filtered from Groups)
    private readonly ObservableCollection<UIElement> simplifiedItems = new();
    ...
    /// <summary>
    /// Gets the collection of items shown in Simplified mode.
    /// This collection is automatically filtered from Groups based on ShowInSimplified property.
    /// </summary>
    public ObservableCollection<UIElement> SimplifiedItems => simplifiedItems;
        ...
        switch (e.Action)
        {
            case NotifyCollectionChangedAction.Add:
            {
                ...
                for (var i = 0; i < e.NewItems?.Count; i++)
                {
                    ...
                    {
                        // Add to SimplifiedItems if ShowInSimplified is true
                        if (ShouldShowInSimplified(element))
                        {
                            simplifiedItems.Add(element);
                        }

                        // Only add to visual tree if we're in the correct mode
                        if (!isSimplified || ShouldShowInSimplified(element))
                        {
                            // In Classic mode: add all
                            // In Simplified mode: add only if ShowInSimplified=true
            ...
            case NotifyCollectionChangedAction.Replace:
            {
...

                {
                    ...
                    foreach (UIElement item in e.NewItems.NullSafe().OfType<UIElement>())
                    {
                        ...
                        // Add to SimplifiedItems if ShowInSimplified is true
                        if (ShouldShowInSimplified(item))
                        {
            ...
            case NotifyCollectionChangedAction.Reset:
            {
                ...
                simplifiedItems.Clear();
                {
                    ...
                    foreach (RibbonGroupBox group in Groups)
                    {
                        ...
                        // Add to SimplifiedItems if ShowInSimplified is true
                        if (ShouldShowInSimplified(group))
                        {
                            simplifiedItems.Add(group);
                        }
    ...
    /// <summary>
    /// Determines whether the specified element should be shown in Simplified mode.
    /// </summary>
    private static bool ShouldShowInSimplified(UIElement element)
    {
        // Check ShowInSimplified property via reflection (works for both IRibbonControl and RibbonGroupBox)
        System.Reflection.PropertyInfo? showInSimplifiedProperty = element.GetType().GetProperty("ShowInSimplified");
        if (showInSimplifiedProperty != null)
        {
            return (bool)(showInSimplifiedProperty.GetValue(element) ?? true);
        }

        // Default: show in simplified mode
        return true;
    }
    ...
    private static void OnIsSimplifiedChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is RibbonTabItem ribbonTabItem)
        {
            ...
            // Update simplified state for all groups
            ...
            // Swap the collection: switch between Groups and SimplifiedItems
            ribbonTabItem.SwapGroupsCollection(isSimplified);
        }
    }

    /// <summary>
    /// Swaps the groups collection between normal and simplified mode.
    /// Uses Visibility to show/hide groups based on ShowInSimplified property.
    /// </summary>
    private void SwapGroupsCollection(bool useSimplified)
    {
        foreach (UIElement item in Groups)
        {
            if (useSimplified)
            {
                // Simplified mode: Show only groups with ShowInSimplified=true
                item.Visibility = ShouldShowInSimplified(item) ? Visibility.Visible : Visibility.Collapsed;
            }
            else
            {
                // Classic mode: Show all groups
