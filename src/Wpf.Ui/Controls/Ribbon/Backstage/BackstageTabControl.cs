...
public class BackstageTabControl : Selector, ILogicalChildSupport
{
    ...
    /// <summary>
    /// Gets or sets get or sets the string format for the content.
    /// </summary>
    ...
    /// <summary>
    /// Gets get or sets the string format for the selected content.
    /// </summary>
    ...
    /// <summary>
    /// Gets the <see cref="DataTemplate"/> which should be used for the selected content
    /// </summary>
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    ...
    /// <summary>
    /// Gets the <see cref="ContentTemplateSelector"/> which should be used for the selected content
    /// </summary>
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    ...
    /// <summary>
    /// Gets or sets a value indicating whether defines if the back button is visible or not.
    /// </summary>
    ...
    /// <summary>
    /// Initializes static members of the <see cref="BackstageTabControl"/> class.
    /// Static constructor
    /// </summary>
    ...
    public BackstageTabControl()
    {
        ...
        this.ContextMenu.Opened += (_, _) => this.ContextMenu.SetCurrentValue(ContextMenu.IsOpenProperty, false);
    ...
    {
        this.SetCurrentValue(ParentBackstageProperty, UIHelper.GetParent<Backstage>(this));
    }

    private void HandleUnloaded(object sender, RoutedEventArgs e)
    {
        this.SetCurrentValue(ParentBackstageProperty, null);
    }
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.F6:
                {
                FocusNavigationDirection focusNavigationDirection = (e.KeyboardDevice.Modifiers & ModifierKeys.Shift) == ModifierKeys.Shift
    ...
    {
        BackstageTabItem? container = this.ItemContainerGenerator.ContainerOrContainerContentFromItem<BackstageTabItem>(this.SelectedItem);
            ...
            || container.Visibility != Visibility.Visible)
        {
            ...
            {
                this.SetCurrentValue(SelectedItemProperty, this.ItemContainerGenerator.ItemFromContainerOrContainerContent(container));
            }
        }
    ...
    private BackstageTabItem? FindNextTabItem(int startIndex, int direction)
    {
        ...
        for (var i = 0; i < this.Items.Count; i++)
        {
            ...
            BackstageTabItem? container = this.ItemContainerGenerator.ContainerOrContainerContentFromIndex<BackstageTabItem>(index);
            if (container is not null
                && container.IsEnabled
                && container.Visibility == Visibility.Visible)
            {
    ...
    /// <inheritdoc />
    protected override AutomationPeer OnCreateAutomationPeer() => new RibbonBackstageTabControlAutomationPeer(this);
