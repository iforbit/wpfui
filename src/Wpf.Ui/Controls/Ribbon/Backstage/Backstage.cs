...
public class Backstage : RibbonControl
{
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether backstage is shown
    /// </summary>
    public bool IsOpen
    {
    ...
    private static void OnIsOpenChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        (UIElementAutomationPeer.FromElement(control) as RibbonBackstageAutomationPeer)?.RaiseExpandCollapseAutomationEvent(oldValue, newValue);
    }
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether backstage can be openend or closed.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether context tabs on the titlebar should be hidden when backstage is open
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether opening or closing should be animated.
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether to close the backstage when Esc is pressed
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether the highest available adorner layer should be used for the <see cref="BackstageAdorner"/>.
    ...
    private static void OnContentChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        {
            if (e.OldValue is DependencyObject oldDependencyObject)
            {
                BindingOperations.ClearBinding(oldDependencyObject, VisibilityProperty);
            }
        ...
        if (e.NewValue is not null)
        {
            ...
            {
                _ = BindingOperations.SetBinding(dependencyObject, VisibilityProperty, new Binding { Path = new PropertyPath(VisibilityProperty), Source = backstage });
            }
        }
    }
    ...
    /// <summary>
    /// Initializes static members of the <see cref="Backstage"/> class.
    /// Static constructor
    /// </summary>
    static Backstage()
    {
    ...
    protected virtual bool Show()
    {
        ...
        if (this.parentRibbon is not null)
        {
            ...
            {
                this.parentRibbon.TabControl.SetCurrentValue(RibbonTabControl.IsDropDownOpenProperty, false);
                this.parentRibbon.TabControl.SetCurrentValue(RibbonTabControl.HighlightSelectedItemProperty, false);
            ...
            // TitleBar integration removed as requested
        }
    ...
    private void ShowAdorner()
    {
        ...
        if (this.AreAnimationsEnabled
            && this.TryFindResource("Wpf.Ui.Storyboards.Backstage.IsOpenTrueStoryboard") is Storyboard storyboard)
        {
        ...
        {
            this.adorner.SetCurrentValue(VisibilityProperty, Visibility.Visible);
        ...
        void MoveFocusToContent()
        {
            ...
            // Focus handling for BackstageTabControl
            if (this.Content is BackstageTabControl { SelectedIndex: not -1 } backstageTabControl
                && backstageTabControl.ItemContainerGenerator.ContainerFromIndex(backstageTabControl.SelectedIndex) is BackstageTabItem backstageTabItem)
            {
                _ = backstageTabItem.Focus();
                return;
            }

            if (this.Content is BackstageTabControl { SelectedContentHost: { } selectedContentHost })
            {
                _ = selectedContentHost.MoveFocus(new TraversalRequest(FocusNavigationDirection.Next));
                return;
            }

            _ = this.Content.MoveFocus(new TraversalRequest(FocusNavigationDirection.Next));
        }
    }
    ...
    private void HideAdornerAndRestoreParentProperties()
    {
        ...
        if (this.AreAnimationsEnabled
            && this.TryFindResource("Wpf.Ui.Storyboards.Backstage.IsOpenFalseStoryboard") is Storyboard storyboard)
        {
            storyboard = storyboard.Clone();
        ...
        {
            this.adorner.SetCurrentValue(VisibilityProperty, Visibility.Collapsed);
        ...
        void HandleStoryboardOnCompleted(object? sender, EventArgs args)
        {
            ...
            {
                this.AdornerLayer.SetCurrentValue(VisibilityProperty, Visibility.Visible);
            }
    ...
    {
        Backstage target = ((BackstageAdorner)args.Source).Backstage;
    ...
    private void CreateAndAttachBackstageAdorner()
    {
        ...
        _ = BindingOperations.SetBinding(this.adorner, DataContextProperty, new Binding
        {
        ...
        _ = this.AdornerLayer.CommandBindings.Add(new CommandBinding(RibbonCommands.OpenBackstage, HandleOpenBackstageCommandExecuted, HandleOpenBackstageCommandCanExecute));
    }
    ...
    private void RestoreParentProperties()
    {
        if (this.parentRibbon is not null)
        {
            if (this.parentRibbon.TabControl is not null)
            {
                this.parentRibbon.TabControl.SetCurrentValue(RibbonTabControl.HighlightSelectedItemProperty, true);
                this.parentRibbon.TabControl.RequestBackstageClose -= this.HandleTabControlRequestBackstageClose;
            }
        ...
        // Uncollapse elements
        foreach (KeyValuePair<FrameworkElement, Visibility> element in this.collapsedElements)
        {
            element.Key.SetCurrentValue(VisibilityProperty, element.Value);
        }
    ...
    /// <inheritdoc />
    protected override AutomationPeer OnCreateAutomationPeer() => new RibbonBackstageAutomationPeer(this);
}
