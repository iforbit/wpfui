...
[TemplatePart(Name = "PART_DropDownButton", Type = typeof(RibbonDropDownButton))]
public class Gallery : ListBox
{
    ...
    /// <summary>
    /// Gets or sets a value indicating whether gets or sets whether the inner gallery panel shows groups
        ...
        switch (e.Action)
        {
            ...
            case NotifyCollectionChangedAction.Remove:
                foreach (GalleryGroupFilter item in e.OldItems.NullSafe().OfType<GalleryGroupFilter>())
                {
                    if (this.groupsMenuButton is not null)
                    {
                        RibbonMenuItem? menuItem = this.GetFilterMenuItem(item);
            ...
            case NotifyCollectionChangedAction.Replace:
                ...
                foreach (GalleryGroupFilter item in e.NewItems.NullSafe().OfType<GalleryGroupFilter>())
                {
                    if (this.groupsMenuButton is not null)
                    {
                        ...
                        menuItem.Click += this.OnFilterMenuItemClick;
                        _ = this.groupsMenuButton.Items.Add(menuItem);
                    }
                }
    ...
    public static readonly DependencyProperty SelectedFilterProperty =
        DependencyProperty.Register(
            ...
            typeof(Gallery),
            new PropertyMetadata(null, OnSelectedFilterChanged, CoerceSelectedFilter));
    ...
    {
        var senderItem = (RibbonMenuItem)sender;
        RibbonMenuItem? item = this.GetFilterMenuItem(this.SelectedFilter);
        if (item is not null)
        {
    ...
    private RibbonMenuItem? GetFilterMenuItem(GalleryGroupFilter? filter)
    {
        if (filter is null)
        {
            return null;
        ...
        return this.groupsMenuButton?.Items.Cast<RibbonMenuItem>().FirstOrDefault(item => (item is not null) && (item.Header.ToString() == filter.Title));
    ...
    /// <summary>
    /// Gets a value indicating whether gets whether gallery is last item in ItemsControl
    /// </summary>
    ...
    public override void OnApplyTemplate()
    {
        if (this.groupsMenuButton is not null)
        {
            foreach (RibbonMenuItem menuItem in this.groupsMenuButton.Items.Cast<RibbonMenuItem>())
            {
                menuItem.Click -= this.OnFilterMenuItemClick;
            }
        }
        ...
        this.groupsMenuButton = this.GetTemplateChild("PART_DropDownButton") as RibbonDropDownButton;
        ...
        if (this.groupsMenuButton is not null)
        {
            for (var i = 0; i < this.Filters.Count; i++)
            {
                ...
                _ = this.groupsMenuButton.Items.Add(item);
            }
        }
