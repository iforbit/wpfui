...
public class RibbonTabItemAutomationPeer : FrameworkElementAutomationPeer
{
    ...
    public override object GetPattern( PatternInterface patternInterface )
    {
        switch (patternInterface)
        {
            case PatternInterface.Scroll:
                System.Windows.Controls.ScrollViewer? container = this.OwningTab.GroupsContainer;
                if (container is not null)
                {
                    AutomationPeer? automationPeer = CreatePeerForElement(container);
    ...
    protected override List<AutomationPeer> GetChildrenCore()
    {
        ...
        foreach (RibbonGroupBox @group in this.OwningTab.Groups)
        {
            AutomationPeer? peer = CreatePeerForElement(@group);
