...
public class RibbonGroupsContainer : Panel, IScrollInfo
{
    ...
    {
        // Separate groups by ColumnIndex
        var leftColumnGroups = new List<UIElement>();
        var rightColumnGroups = new List<UIElement>();
        ...
        foreach (UIElement? item in this.InternalChildren)
        {
            ...
            int columnIndex = RibbonProperties.GetColumnIndex(item);
            if (columnIndex == 1)
            {
                rightColumnGroups.Add(item);
            }
            else
            {
                leftColumnGroups.Add(item);
            }
        }

        // Calculate total width of each column
        double leftColumnWidth = leftColumnGroups.Sum(x => x.DesiredSize.Width);
        double rightColumnWidth = rightColumnGroups.Sum(x => x.DesiredSize.Width);

        // Reserve space for Simplified toggle button (approximately 40px)
        const double rightMargin = 40;
        double availableWidth = finalSize.Width - rightMargin;

        // Arrange left column groups from left to right
        var leftRect = new Rect(finalSize)
        {
            X = -this.HorizontalOffset
        };

        foreach (UIElement item in leftColumnGroups)
        {
            leftRect.Width = item.DesiredSize.Width;
            leftRect.Height = Math.Max(finalSize.Height, item.DesiredSize.Height);
            item.Arrange(leftRect);
            leftRect.X += item.DesiredSize.Width;
        }

        // Arrange right column groups from right to left
        double rightX = availableWidth - this.HorizontalOffset - rightColumnWidth;
        var rightRect = new Rect(finalSize)
        {
            X = rightX
        };

        foreach (UIElement item in rightColumnGroups)
        {
            rightRect.Width = item.DesiredSize.Width;
            rightRect.Height = Math.Max(finalSize.Height, item.DesiredSize.Height);
            item.Arrange(rightRect);
            rightRect.X += item.DesiredSize.Width;
        }
