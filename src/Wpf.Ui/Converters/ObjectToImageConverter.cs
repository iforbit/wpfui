...
public class ObjectToImageConverter : MarkupExtension, IValueConverter, IMultiValueConverter
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ObjectToImageConverter"/> class.
    ...
    /// <summary>
    /// Gets or sets the target visual on which the image/icon should be shown.
    /// </summary>
    [ConstructorArgument("targetVisualBinding")]
    public Binding? TargetVisualBinding { get; set; }

    /// <summary>
    /// Gets or sets the binding to which the converter should be applied to.
    /// </summary>
    [ConstructorArgument("iconBinding")]
    public Binding? IconBinding { get; set; }

    /// <summary>
    /// Gets or sets the binding for the desired size for the image.
    /// </summary>
    ...
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo? culture)
    {
        ...
        return Convert(value, null, desiredSize, targetType);
    }
    ...
    public object? Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        Visual? targetVisual = valuesLength == 2
            ? values[1] as Visual
            : null;
        ...
        return Convert(values[0], targetVisual, desiredSize, targetType);
    }
    ...
    private object CreateMultiBinding(IServiceProvider serviceProvider)
    {
        ...
        multiBinding.Bindings.Add(IconBinding);

        if (DesiredSizeBinding is not null)
        {
            multiBinding.Bindings.Add(DesiredSizeBinding);
        }

        if (TargetVisualBinding is not null)
        {
            multiBinding.Bindings.Add(TargetVisualBinding);
        }
    ...
    private class ServiceProviderWrapper : IServiceProvider, IProvideValueTarget
    {
        ...
        /// <inheritdoc />
        public object? TargetObject => ((IProvideValueTarget)serviceProvider).TargetObject is DependencyObject dpo
            ? dpo
            : null;

        /// <inheritdoc />
        public object TargetProperty => ((IProvideValueTarget)serviceProvider).TargetProperty;
    }

    private object? Convert(object? value, Visual? targetVisual, Size desiredSize, Type targetType)
    {
        ImageSource? imageSource = CreateFrozenImageSource(GetValueToConvert(value, desiredSize, targetVisual), targetVisual, desiredSize);
    ...
    public static ImageSource? CreateImageSource(object? value, Visual? targetVisual, Size desiredSize)
    {
        ...
        // Hack to get the value from resource expressions
        {
                ...
                && value is Expression expression)
            {
                ...
                MethodInfo? method = type.GetMethod("GetValue", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                FieldInfo? field = type.GetField("_targetObject", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
    ...
    {
        BitmapSource imageSource = Imaging.CreateBitmapSourceFromHIcon(icon.Handle, Int32Rect.Empty, BitmapSizeOptions.FromEmptyOptions());
    ...
    {
        Size scaledDesiredSize = GetScaledDesiredSize(desiredSize, targetVisual);
